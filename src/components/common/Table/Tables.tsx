...
function Tables() {
  ...
  useEffect(() => {
    const subscription = client.models.UptimeProjects.observeQuery().subscribe({
      next: ({ items, isSynced }) => {
  ...
  const CheckDeleteButton = selectedProject?.length !== 0;
    ...
    <div className="w-full  border border-gray-200 shadow-md rounded-[8px] p-2">
      ...
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h1 className="font-bold text-[24px]">Tables</h1>

          <div className="flex gap-2 items-center">
            <div
              className="rounded-full p-1 border-[1.5px] border-[#1B76E2] hover:bg-blue-50"
              onClick={() => {
                dispatch(fetchProjects());
              }}
            >
              <RotateCw size={15} className="text-[#1B76E2] cursor-pointer " />
            </div>
          ...
          <thead className="border-b border-gray-200">
            <tr className="text-left">
              ...
              {tabletitleList.map((item) => (
                <TableHead key={item.id} title={item.title} />
              ))}
            </tr>
          </thead>
